...
class OpenforumForm(object):

    def __init__(self, driver):
        self.driver = driver

    def fill_form(self, number):
        self.fill_open_forum_name("Open Forum" + str(number))
        self.save_and_return()

    def fill_open_forum_name(self, name):
        element = WebDriverWait(self.driver, 10). \
            until(EC.element_to_be_clickable(PageLocators.OPEN_FORUM_NAME))
        element.clear()
...
class PageLocators(object):
    OPEN_FORUM_NAME = (By.CSS_SELECTOR, 'input[id="id_name"]')
